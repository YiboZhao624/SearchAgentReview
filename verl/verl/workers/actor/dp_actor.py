        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # Collect scatter data using ORIGINAL rollout old_log_probs (not reassigned on-policy)
                    # and current model's log_prob to get the actual ratio for visualization
                    if not scatter_data_collected:
                        original_old_log_prob = model_inputs["old_log_probs"]
                        _response_mask = model_inputs["response_mask"]
                        _mask_bool = _response_mask.bool()
                        _ratio = torch.exp(log_prob - original_old_log_prob).detach()
                        _old_prob = torch.exp(original_old_log_prob).detach()
                        metrics["actor/_scatter_old_prob"] = _old_prob[_mask_bool].cpu().numpy()
                        metrics["actor/_scatter_ratio"] = _ratio[_mask_bool].cpu().numpy()
                        metrics["actor/_scatter_advantages"] = advantages.detach()[_mask_bool].cpu().numpy()
                        scatter_data_collected = True
                    # Remove any scatter keys from pg_metrics (in case policy loss fn adds them)
                    for k in list(pg_metrics.keys()):
