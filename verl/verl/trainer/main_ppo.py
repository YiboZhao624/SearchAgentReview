...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # Forward multi_turn.format into data config so dataset classes can
        # select the matching prompt template (e.g. "hermes" vs "search_r1").
        multi_turn_format = config.get("actor_rollout_ref", {}).get("rollout", {}).get("multi_turn", {}).get("format")
        if multi_turn_format is not None:
            from omegaconf import OmegaConf

            OmegaConf.update(config, "data.prompt_format", multi_turn_format, force_add=True)
