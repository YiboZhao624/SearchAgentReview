...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    if "tool_call_failure_counts" in batch.non_tensor_batch:
        tool_call_failure_counts = batch.non_tensor_batch["tool_call_failure_counts"]
        metrics["tool_call_failure_counts/min"] = tool_call_failure_counts.min()
        metrics["tool_call_failure_counts/max"] = tool_call_failure_counts.max()
        metrics["tool_call_failure_counts/mean"] = tool_call_failure_counts.mean()
        metrics["tool_call_failure_counts/sum"] = tool_call_failure_counts.sum()

    # ── Success-split fine-grained metrics ──────────────────────────────────
    # "success" comes from reward_fn via reward_extra_info dict.
    # Depending on TensorDict version and dtype, it may land in batch.batch (torch tensor)
    # or batch.non_tensor_batch (numpy array via NonTensorStack).
    _success_arr: np.ndarray | None = None
    if "success" in batch.batch:
        _success_arr = batch.batch["success"].cpu().numpy().astype(np.int64)
    elif "success" in batch.non_tensor_batch:
        _success_arr = np.asarray(batch.non_tensor_batch["success"], dtype=np.int64)

    if _success_arr is not None:
        metrics.update(
            _compute_success_split_metrics(
                batch=batch,
                success_arr=_success_arr,
...
def _compute_success_split_metrics(
    batch: "DataProto",
    ...
    """
    Compute fine-grained metrics split by the per-sample success flag.

    Args:
        success_arr: 1-D int64 numpy array (values 0 or 1), one entry per sample.
          - 1 : the rollout was marked as successful by the reward function
          - 0 : the rollout was marked as failed

    This field is populated by the reward function via ``reward_extra_info``:

        def my_reward_fn(data, return_dict=True):
            ...
            return {
                "reward_tensor": reward_tensor,
                "reward_extra_info": {
                    "success": [1, 0, 1, ...],   # int list, one per sample
                },
            }

    Optionally uses:
      - ``batch.non_tensor_batch["uid"]``                     – prompt-level group id (for group-wise stats)
      - ``batch.non_tensor_batch["__num_turns__"]``            – dialogue turn count
      - ``batch.non_tensor_batch["tool_call_counts"]``         – total tool calls per sample
      - ``batch.non_tensor_batch["tool_call_failure_counts"]`` – failed tool calls per sample
    """
    metrics: dict[str, Any] = {}

    success_flags = success_arr.astype(bool)  # True = success
    fail_flags = ~success_flags
    n_success = int(success_flags.sum())
    n_fail = int(fail_flags.sum())

    # ── 1. Overall success rate ───────────────────────────────────────────
    metrics["success/rate"] = float(success_flags.mean())

    # ── Helpers ───────────────────────────────────────────────────────────
    resp_len_np = response_length.cpu().numpy()     # (batch_size,)
    seq_score_np = sequence_score.cpu().numpy()     # (batch_size,)
    seq_reward_np = sequence_reward.cpu().numpy()   # (batch_size,)

    group_labels = [("success", success_flags), ("fail", fail_flags)]

    # ── 2. Response length by group ───────────────────────────────────────
    for label, mask in group_labels:
        n = int(mask.sum())
        if n == 0:
            continue
        rl = resp_len_np[mask]
        metrics[f"{label}/response_length/mean"] = float(rl.mean())
        metrics[f"{label}/response_length/max"] = float(rl.max())
        metrics[f"{label}/response_length/min"] = float(rl.min())
        metrics[f"{label}/response_length/clip_ratio"] = float((rl == max_response_length).mean())

    # ── 3. Score and reward by group ──────────────────────────────────────
    for label, mask in group_labels:
        n = int(mask.sum())
        if n == 0:
            continue
        metrics[f"{label}/score/mean"] = float(seq_score_np[mask].mean())
        metrics[f"{label}/reward/mean"] = float(seq_reward_np[mask].mean())

    # ── 4. Num turns by group ─────────────────────────────────────────────
    if "__num_turns__" in batch.non_tensor_batch:
        num_turns_np = batch.non_tensor_batch["__num_turns__"]
        for label, mask in group_labels:
            n = int(mask.sum())
            if n == 0:
                continue
            nt = num_turns_np[mask]
            metrics[f"{label}/num_turns/mean"] = float(nt.mean())
            metrics[f"{label}/num_turns/max"] = float(nt.max())
            metrics[f"{label}/num_turns/min"] = float(nt.min())

    # ── 5. Tool call counts by group ──────────────────────────────────────
    tcc_np = None
    if "tool_call_counts" in batch.non_tensor_batch:
        tcc_np = batch.non_tensor_batch["tool_call_counts"]
        for label, mask in group_labels:
            n = int(mask.sum())
            if n == 0:
                continue
            tc = tcc_np[mask]
            metrics[f"{label}/tool_call_counts/mean"] = float(tc.mean())
            metrics[f"{label}/tool_call_counts/max"] = float(tc.max())
            metrics[f"{label}/tool_call_counts/min"] = float(tc.min())
            # Response tokens per tool call (search efficiency)
            total_tc = int(tc.sum())
            if total_tc > 0:
                metrics[f"{label}/tokens_per_tool_call"] = float(resp_len_np[mask].sum()) / total_tc

    # ── 6. Tool call failure counts by group ──────────────────────────────
    if "tool_call_failure_counts" in batch.non_tensor_batch:
        tcfc_np = batch.non_tensor_batch["tool_call_failure_counts"]
        for label, mask in group_labels:
            n = int(mask.sum())
            if n == 0:
                continue
            tcfc = tcfc_np[mask]
            metrics[f"{label}/tool_call_failure_counts/mean"] = float(tcfc.mean())
            # Tool failure rate = failed_calls / total_calls
            if tcc_np is not None:
                total_tcc = int(tcc_np[mask].sum())
                if total_tcc > 0:
                    metrics[f"{label}/tool_failure_rate"] = float(tcfc.sum()) / total_tcc

    # ── 7. Per-prompt group statistics (requires uid) ─────────────────────
    if "uid" in batch.non_tensor_batch:
        uid_arr = batch.non_tensor_batch["uid"]
        unique_uids = np.unique(uid_arr)
        group_success_rates = np.array(
            [float(success_flags[uid_arr == u].mean()) for u in unique_uids],
            dtype=np.float64,
        )
        metrics["group/success_rate/mean"] = float(group_success_rates.mean())
        metrics["group/success_rate/std"] = float(group_success_rates.std())
        metrics["group/success_rate/min"] = float(group_success_rates.min())
        metrics["group/success_rate/max"] = float(group_success_rates.max())
        # Prompts where every rollout failed / succeeded
        metrics["group/all_fail_ratio"] = float((group_success_rates == 0.0).mean())
        metrics["group/all_success_ratio"] = float((group_success_rates == 1.0).mean())
