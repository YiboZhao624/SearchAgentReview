    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "tool_call_failure_counts" in batch_meta.field_names:
                    compute_data_metrics_fields.append("tool_call_failure_counts")
                # "success" is populated by reward_fn via reward_extra_info (int tensor, 1=success, 0=fail)
                if "success" in batch_meta.field_names:
                    compute_data_metrics_fields.append("success")
                # "uid" is needed for per-prompt group success rate statistics
                if "uid" in batch_meta.field_names:
                    compute_data_metrics_fields.append("uid")
