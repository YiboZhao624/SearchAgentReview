...
def build_search_r1_tool_response_text(messages: list[dict[str, Any]]) -> str:
    """Build Search-R1 tool response text: \\n\\n<information>Doc N(Title: X) text\\n...</information>\\n\\n"""
    ...
    for tool_msg in messages:
        ...
        # Tool response is JSON (list of dicts or nested list)
        try:
            parsed = _json.loads(content)
        except (_json.JSONDecodeError, TypeError):
            # Fallback: treat as plain text
            docs.append(f"Doc {doc_idx}(Title: N/A) {content}")
            doc_idx += 1
            continue

        # Flatten: could be list[dict] or list[list[dict]]
        hit_list: list[dict] = []
        if isinstance(parsed, list):
            for item in parsed:
                if isinstance(item, list):
                    hit_list.extend(item)
        ...
        for hit in hit_list:
            ...
            text = hit.get("text", "")
            docs.append(f"Doc {doc_idx}(Title: {title}) {text}")
            doc_idx += 1
