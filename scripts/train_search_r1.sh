#!/usr/bin/env bash
# Search-R1 baseline training script
# Advantage estimator: standard GRPO
...
EXPERIMENT_NAME="$(date +%m%d)-Qwen3-8B-SearchR1"
...
CONFIG_NAME="${CONFIG_NAME:-search_r1}"
...
ray start --head --port="${RAY_PORT}" --dashboard-port 8266
