#!/usr/bin/env bash
# IGPO training script
# Advantage estimator: grpo_igpo (information gain weighted advantage)
# Reward: igpo_compute_score (EM/F1 + info gain)
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7

set -euo pipefail

PROJECT_NAME="igpo"
EXPERIMENT_NAME="$(date +%m%d)-Qwen3-8B-IGPO"
...
python -m verl.trainer.main_ppo \
  ...
  algorithm.adv_estimator=grpo_igpo \
  algorithm.compute_info_gain=true \
  algorithm.info_gain_type=log_prob_diff \
  custom_reward_function.path="${ROOT_DIR}/src/reward/igpo_info_gain.py" \
  custom_reward_function.name="igpo_compute_score" \
  actor_rollout_ref.rollout.multi_turn.dynamic_prompt.enable=false \
  actor_rollout_ref.rollout.multi_turn.custom_tooling.enable=false \
